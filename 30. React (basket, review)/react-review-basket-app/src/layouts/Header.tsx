...
export function Navbar() {
  ...
  const registerFormik = useFormik({
    ...
    onSubmit: (values, actions) => {
      ...
      const registredUser = instance.post(endpoints.users, newUser);
        ...
        <nav className="font-inter mx-auto h-auto w-full max-w-screen-2xl md:relative md:top-0">
          <div className="flex flex-col px-6 py-6 md:flex-row md:items-center md:justify-between md:px-10 md:py-4 xl:px-20">
                ...
                routes.map((route, idx) => {
                  return (
                    <NavLink
                      ...
                      to={route.url}
                      className={({ isActive }) => {
                        return `flex font-bold text-lg text-gray-600 flex-row rounded-lg md:px-6 md:py-4 ${
                          isActive && "text-gray-950 underline"
                        }`;
                      }}
                    >
                      {route.title}
            ...
            <div className="flex flex-col space-y-8 md:flex md:flex-row md:space-x-3 md:space-y-0">
              {!user ? (
                <>
                  <button className="font-inter rounded-lg md:px-6 md:py-4 md: md:hover:text-gray-800">
                    <Dialog>
                      <DialogTrigger>Register</DialogTrigger>
                      <DialogContent>
                        <DialogHeader>
                          <DialogTitle className="flex justify-center gap-2 text-center my-3 text-xl">
                            Sign Up <Lock />
                          </DialogTitle>
                          <DialogDescription>
                            <form
                            ...
                            >
                              <input
                                className="border border-gray-900 font-bold text-black rounded px-2 py-4"
                                type="email"
                                name="email"
                              ...
                              <button
                                type="submit"
                                className="bg-gray-600 rounded px-2 py-4 text-white"
                              >
                                sign up
                              </button>
                ...
                <div className="flex items-center gap-4">
                  <h1>
                    hello <span className="font-bold">{user.username} <sup>{user.basketItems.length}</sup></span>
                  </h1>
                  <button
